...
export type FieldBuffers = {
  ...
  // “surface targets” kept for compatibility, but here they’re all zero
  theta_surf_target: GPUBuffer;
  qv_surf_target: GPUBuffer;

  // packed thermo bg: [theta0, dtheta0_dz, qv_bg]
  bg_thermo: GPUBuffer;
};
...
export async function initBoussinesq3D(device: GPUDevice) {
  ...
  // Surface target fields (we’ll set them to zero everywhere for this experiment)
  const theta_surf_target = new Float32Array(N);
  const qv_surf_target = new Float32Array(N);
  ...
  // ------------------ Constants (match-ish your Python) ------------------
  const theta0_surface = 300.0;             // K

  // New: BL + inversion structure
  const dtheta_bl  = 0.001;                // 1 K/km below cap (weakly stable / quasi-neutral)
  const dtheta_cap = 0.010;                // 10 K/km above cap (strong inversion)
  const z_cap      = 1200.0;               // base of inversion (m)
  ...
  // New: BL vs FT humidity
  const RH_bl  = 0.8;                       // moist BL
  const RH_ft  = 0.3;                       // drier free troposphere
  const z_bl   = 1500.0;                    // top of moist BL (m)

  // Bubble (ellipsoidal), placed just below inversion
  const xb = Lx * 0.5, yb = Ly * 0.5, zb = 1200.0;
  const rb_xy = 800.0, rb_z = 400.0;
  const RH_bubble = 0.95;
  const theta_amp = 1.5;                    // K anomaly for bubble

  // Surface forcing params — effectively OFF for this experiment
  const Nbl = 0;                            // no bottom levels forced
  const tau_surf = 1.0e9;                   // huge, so surface relaxation is negligible
  ...
  // Shear: 0 m/s at surface → 8 m/s at 4 km
  const U_sfc = 0.0;
  const U_top = 2.0;

  // Noise amplitudes to break symmetry outside bubble
  const noiseTheta = 0.05;     // 0.05 K
  const noiseQ     = 5e-5;     // small qv noise
  ...
  for (let k = 0; k < nz; k++) {
    ...
    // Background theta profile with BL + inversion
    let thetabase_k: number;
    let dtheta_k: number;

    if (z < z_cap) {
      thetabase_k = theta0_surface + dtheta_bl * z;
      dtheta_k    = dtheta_bl;
    } else {
      const theta_cap = theta0_surface + dtheta_bl * z_cap;
      thetabase_k = theta_cap + dtheta_cap * (z - z_cap);
      dtheta_k    = dtheta_cap;
    }

    const pbase_k = p_surf * Math.exp(-z / H);  // p0(z)
    const Pi_k    = exner(pbase_k, p_ref, R, cp);
    const T0_k    = thetabase_k * Pi_k;         // T0(z)

    const qs_bg_k = saturationMixingRatio(T0_k, pbase_k, eps);

    // RH profile: moist BL, drier FT
    const wRH  = Math.min(z / z_bl, 1.0);       // 0 at surface, 1 at/above BL top
    const RH_k = RH_bl + (RH_ft - RH_bl) * wRH;
    const qv_bg_k = RH_k * qs_bg_k;

    // Background shear
    const shear = (U_top - U_sfc) / Lz;
    const u_bg  = U_sfc + shear * z;
    ...
    for (let j = 0; j < ny; j++) {
      ...
      for (let i = 0; i < nx; i++) {
        ...
        p0[id]           = pbase_k;
        qv_bg[id]        = qv_bg_k;
        dtheta0_dz_arr[id] = dtheta_k;

        // bubble mask (ellipsoidal)
        const r2_xy = (x - xb) * (x - xb) + (y - yb) * (y - yb);
        const r2_z  = (z - zb) * (z - zb);
        const bubble = Math.exp(
          -r2_xy / (2.0 * rb_xy * rb_xy) -
          r2_z  / (2.0 * rb_z  * rb_z)
        );

        // initial theta perturbation (warm bubble)
        ...
        // velocity: shear in u, rest zero initially
        u[id]  = u_bg;
        v[id]  = 0.0;
        w[id]  = 0.0;

        // no initial cloud or Exner perturbation
        qc[id] = 0.0;
        pi[id] = 0.0;

        // Add tiny noise outside the core bubble to break symmetry
        const rnd = (Math.random() - 0.5);  // [-0.5, 0.5]
        const noiseMask = 1.0 - Math.min(bubble, 1.0); // ~1 away from bubble, 0 in core

        theta_p[id] += noiseMask * noiseTheta * rnd;
        qv[id]      += noiseMask * noiseQ     * rnd;

        // Surface targets: OFF (Nbl = 0, so always zero)
        theta_surf_target[id] = 0.0;
        qv_surf_target[id]    = 0.0;

        // rhs arrays start at 0
        rhs_w_arr[id]        = 0.0;
        rhs_theta_p_arr[id]  = 0.0;
        rhs_qv_arr[id]       = 0.0;
        rhs_qc_arr[id]       = 0.0;
        rhs_u_arr[id]        = 0.0;
        rhs_v_arr[id]        = 0.0;
  ...
  // Pack thermo info: [theta0, dtheta0_dz, qv_bg]
  thermoPacked.set(theta0,         0 * N);
  thermoPacked.set(dtheta0_dz_arr, 1 * N);
  thermoPacked.set(qv_bg,          2 * N);
  ...
  const fields: FieldBuffers = {
    ...
    rhs_u: makeBuf(rhs_u_arr),
    rhs_v: makeBuf(rhs_v_arr),
    theta_surf_target: makeBuf(theta_surf_target),
    qv_surf_target: makeBuf(qv_surf_target),
    bg_thermo: makeBuf(thermoPacked),
  };

  const params: Params = {
    Lv, cp, R, p_ref, eps, g,
    tau_damp_w, tau_rad, Nbl, tau_surf,
    qc_crit, rain_frac,
    dx, dy, dz,
    nu, kappa, Dq,
  };
